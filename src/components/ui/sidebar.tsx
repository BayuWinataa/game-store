...
export const DesktopSidebar = ({ className, children, ...props }: React.ComponentProps<typeof motion.div>) => {
	...
	return (
		<>
			<motion.div
				className={cn('h-full px-3 py-4 hidden md:flex md:flex-col w-[300px] shrink-0', className)}
				animate={{
...
export const MobileSidebar = ({ className, children, ...props }: React.ComponentProps<'div'>) => {
	...
	return (
		<>
			<div className={cn('h-10 px-4 py-4 flex flex-row md:hidden items-center justify-between w-full bg-white/0', className)} {...props}>
...
export const SidebarLink = ({ link, className, ...props }: { link: Links; className?: string }) => {
	...
	return (
		<a href={link.href} className={cn('flex items-center justify-start gap-3 group/sidebar py-2 px-2 rounded-md hover:bg-white/10 transition-colors duration-200', className)} {...props}>
			<div className="h-6 w-6 flex items-center justify-center text-white/90">{link.icon}</div>
			...
			<motion.span
				...
				className="text-white/90 text-base font-medium group-hover/sidebar:translate-x-1 transition duration-150 whitespace-pre inline-block !p-0 !m-0"
			>
				{link.label}
			</motion.span>
		</a>
